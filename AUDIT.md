...
- **Evidence:** `OgmaEmbedder.run` calls `out = try model.prediction(from: input)` synchronously; `OgmaEmbedder.embed`/`embed(_:query:)` are plain synchronous functions. Every caller is `@MainActor`: `ClipStore.add` runs `ClipIndexer.index` inline on the 0.4s poll path (`ClipboardMonitor.poll` -> `store.add`); `PanelViewModel.results` calls `TagSpace.nearestTag` and `SemanticRanker.essence` on every keystroke; `TagSpace.vectors` embeds the active basket's 48 hybrid-axis/topical tag names in a loop (cached per (signature, basket)). Default tier is `.normal` (ogma-small), so this runs out of the box when a model is bundled.
